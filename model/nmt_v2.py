...
'''
    ...
    Description: Char level implementation of nmt
    Resource:
'''
...
with tf.variable_scope('root'):
    ...
    decoder_emb_inp = tf.nn.embedding_lookup(
            embedding_decoder, decoder_inputs) # encoder_outputs can be wrong
...
with tf.variable_scope('root', reuse=True):
    # Placeholders
    infer_encoder_inputs = tf.placeholder(tf.int32, (inp_seq_length, batch_size), 'inputs')
    infer_targets = tf.placeholder(tf.int32, (dec_inp_seq_length, batch_size), 'targets')
    infer_seqlen = tf.placeholder(tf.int32, [batch_size], 'seqlen')
    infer_target_seqlen = tf.placeholder(tf.int32, [batch_size], 'target_seqlen')
    #print("targets ", targets.shape)

    # Embedding
    infer_embedding_encoder = tf.get_variable(
            "embedding_encoder", [inp_alphabet_size, embedding_size])
    infer_encoder_emb_inp = tf.nn.embedding_lookup(
            infer_embedding_encoder, infer_encoder_inputs)


    infer_embedding_decoder = tf.get_variable(
            "embedding_decoder", [out_alphabet_size, embedding_size])

    # Encoder graph
    # lstm cell
    infer_encoder_cell = tf.nn.rnn_cell.BasicLSTMCell(num_units)

    # Dynamic RNN
    # encoder_outputs: [max_time, batch_size, num_units]
    # encoder_state: [batch_size, num_unis]
    infer_encoder_outputs, infer_encoder_state = tf.nn.dynamic_rnn(
            infer_encoder_cell, infer_encoder_emb_inp, 
            sequence_length=infer_seqlen, time_major=True, dtype=tf.float32)


    # Decoder graph
    # lstm cell 
    infer_decoder_cell = tf.nn.rnn_cell.BasicLSTMCell(num_units)

    # Greedy helper
    start_tokens = tf.tile([vezins_dict[start_token_str]], [batch_size])
    end_token = vezins_dict[end_token_str]

    infer_helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(embedding=infer_embedding_decoder, start_tokens=start_tokens, end_token=end_token)


    # Projection layer
    #projection_layer = layers_core.Dense(
    #       out_alphabet_size , use_bias=False) # the out_alphabet_size can be wrpng parameter
    infer_projection_layer = layers_core.Dense(
        out_alphabet_size, use_bias=False)


    # decoder
    infer_decoder = tf.contrib.seq2seq.BasicDecoder(
        infer_decoder_cell, infer_helper, infer_encoder_state, 
        output_layer=infer_projection_layer)

    maximum_iterations = tf.round(tf.reduce_max(dec_inp_seq_length) * 2)
    # Dynamic decoding
    infer_outputs, _, _ = tf.contrib.seq2seq.dynamic_decode(infer_decoder, maximum_iterations=dec_inp_seq_length, output_time_major=False)
    infer_logits = infer_outputs.rnn_output

    print("dec_inp_seq_length :" , dec_inp_seq_length)
    ...
    infer_print = tf.Print(logits_shape, [logits_shape], message="Infer logits: ")
    ...
    """
    # Loss
    infer_target_output = tf.transpose(infer_targets)
    ...
    infer_target_weights = tf.sequence_mask(
        infer_target_seqlen, dec_inp_seq_length, dtype=infer_logits.dtype)
...
for i in range(iter_size):
    try:
        ...
        if i%100 == 0:
            ...
            print("Loss : ", loss)
            encoder_inp, decoder_inp, trgt, seq_length, trgt_seq_length, t= iterator.test_next()
